...
if (( main_alive || watch_alive )); then
  ...
  echo "---"
  echo "Allow Sleep | bash=$SCRIPT param0=off terminal=false refresh=true"
  if (( watch_alive )); then
    echo "Stop Watching | bash=$SCRIPT param0=unwatch terminal=false refresh=true"
  fi
else
  echo "○"
  echo "---"
  ...
  echo "---"
  echo "Keep Awake | bash=$SCRIPT param0=on terminal=false refresh=true"
  echo "Watch Pi | bash=$SCRIPT param0=watch terminal=false refresh=true"
fi
