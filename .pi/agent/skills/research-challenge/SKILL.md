...
description: Stress-test a research project's theoretical motivation, methods, design, and contribution claims before the design is locked in. Use at early project stages to pressure-test decisions that are still open. Works best after running research-search. Can target a specific aspect (theory, methods, contribution) or cover all four dimensions.
allowed-tools: Read, Bash, Grep, question, web_search, mcp__zotero__zotero_search_items, mcp__mcp-research__search_papers, mcp__mcp-research__get_paper_details, mcp__mcp-research__get_paper_citations, mcp__mcp-research__get_paper_references, todo
---
...
Check the user's message for a file path to review or a specific aspect to
focus on (e.g., "methods", "theory", "contribution").

If a file path is given, read it as the primary source.

If a specific aspect is named, note it — you will focus your critique there
but still read the full design for context.
...
If the project has too little documented to critique, use the `question` tool
to ask the user to describe their research design. Include these as labeled
options or allow free-text entry:
- "Describe your research question and theoretical framing"
- "Describe your proposed methods"
- "Describe your expected contribution"
- "Paste a short description of the full design"
...
If the user named a specific aspect, go deep on that dimension and treat the
...
Use the `todo` tool (action: `add`) to register actionable follow-up items:

- One task per major challenge that requires a design decision or
  additional justification.
- Include tasks for literature the researcher should read, e.g.,
  "Read [Author Year] on [competing framework] and assess fit".
- Include tasks for design modifications worth considering.
- Note ordering dependencies in the task text itself where needed, e.g.,
  "[after: read competing framework literature]".
- Use imperative subjects: "Justify choice of X over Y" not "Framework concerns".

Use `todo` (action: `list`) at the end so the user can see the full set.
