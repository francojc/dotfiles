...
Use Pi's `bash` tool only for `hunk session *` commands. Never start, replace, or automate Hunk's user-owned interactive TUI. `hunk diff`, `hunk show`, `hunk patch`, `hunk pager`, and `hunk difftool` are interactive commands for the user.
...
- **"No active Hunk sessions"** -- confirm Hunk is open in the user's terminal. If it is, inspect local daemon/session availability; do not start or automate the interactive TUI.
