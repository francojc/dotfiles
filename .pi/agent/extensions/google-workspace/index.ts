...
const PASS_CLIENT_ID_ENTRY = "GWS/GOOGLE_CLIENT_ID";
const PASS_CLIENT_SECRET_ENTRY = "GWS/GOOGLE_CLIENT_SECRET";
const OAUTH_SCOPE = [
...
// On-disk config holds ONLY tokens + redirectUri. Client credentials come
// from env vars or `pass` and are never written to disk by this extension.
type StoredConfig = {
	redirectUri: string;
	tokens: OAuthTokens;
};
...
async function readPassSecret(entry: string): Promise<string | undefined> {
	try {
		const { stdout } = await execFileAsync("pass", ["show", entry], { timeout: 3_000 });
...
async function resolveAppCredentials(): Promise<AppCredentials | undefined> {
	const clientId = process.env.GOOGLE_CLIENT_ID?.trim() || (await readPassSecret(PASS_CLIENT_ID_ENTRY));
	const clientSecret = process.env.GOOGLE_CLIENT_SECRET?.trim() || (await readPassSecret(PASS_CLIENT_SECRET_ENTRY));
...
async function readConfig(): Promise<StoredConfig | null> {
	try {
		...
		if (!parsed?.redirectUri || !parsed?.tokens?.access_token) return null;
		return parsed;
	} catch {
		return null;
	}
...
async function refreshToken(config: StoredConfig, signal?: AbortSignal): Promise<StoredConfig> {
	const credentials = await requireAppCredentials();
...
async function requireAppCredentials(): Promise<AppCredentials> {
	const credentials = await resolveAppCredentials();
	if (!credentials) {
		throw new Error(
			`Google OAuth client credentials not found. Set GOOGLE_CLIENT_ID/GOOGLE_CLIENT_SECRET or pass entries ${PASS_CLIENT_ID_ENTRY}/${PASS_CLIENT_SECRET_ENTRY}, then run /gws-setup.`,
		);
	}
	return credentials;
}

async function getValidConfig(signal?: AbortSignal): Promise<StoredConfig> {
...
export default function googleWorkspaceExtension(pi: ExtensionAPI) {
		...
		handler: async (_args, ctx) => {
			...
			if (existing) {
				const overwrite = await ctx.ui.confirm(
					...
					`A tokens file already exists. Overwrite it?\n${CONFIG_PATH}`,
				);
				if (!overwrite) return;
			}

			// Client credentials come from env/pass; prompt only as fallback.
			let credentials = await resolveAppCredentials();
			if (!credentials) {
				ctx.ui.notify(
					`No credentials in env/pass (${PASS_CLIENT_ID_ENTRY}, ${PASS_CLIENT_SECRET_ENTRY}). Enter them interactively; they will NOT be saved to disk.`,
					"warning",
				);
			...
			const url = authUrl({ clientId: credentials.clientId, redirectUri, state });
			...
			try {
				...
				ctx.ui.notify(`Tokens saved: ${CONFIG_PATH}`, "info");
			} catch (error) {
	...
	pi.registerTool({
		...
		async execute(): Promise<{ content: Array<{ type: "text"; text: string }>; details: JsonMap }> {
			const config = await readConfig();
			if (!config) {
				return {
