...
- `/session-sync-status` reads the explicit `~/.pi/agent/session-sync.json` source/destination allowlist, checks noninteractive SSH, remote `$HOME`, remote `rsync`, remote Pi session-registry presence, and registry session count. It does not transfer session files.
